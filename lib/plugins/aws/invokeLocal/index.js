...
const envVarResolvers = {
  'AWS::S3::Bucket': require('./resolvers/s3Bucket'),
  'AWS::DynamoDB::Table': require('./resolvers/dynamoDbTable'),
  'AWS::SNS::Topic': require('./resolvers/snsTopic'),
  'AWS::SQS::Queue': require('./resolvers/sqsQueue'),
  'AWS::Lambda::Function': require('./resolvers/lambdaFunction'),
  'AWS::SSM::Parameter': require('./resolvers/ssmParameter'),
  'AWS::StepFunctions::StateMachine': require('./resolvers/stepFunctionsStateMachine'),
};
...
class AwsInvokeLocal {
  constructor(serverless, options) {
    ...
    this.resolveExports = memoizee(this.resolveExports, {
      normalizer: ([value]) => JSON.stringify(value),
    });
    this.resolveResource = memoizee(this.resolveResource, {
      normalizer: ([value]) => JSON.stringify(value),
    });
    this.getCFResources = memoizee(this.getCFResources);
  ...
  loadEnvVars() {
    return BbPromise.try(() => {
      ...
      const promises = _.entries(configuredEnvVars).map(([name, value]) => {
        return this.resolveResource(value).then(output => {
          configuredEnvVars[name] = output;
        });
      });

      return BbPromise.all(promises).then(() => {
        Object.assign(process.env, lambdaDefaultEnvVars, credentialEnvVars, configuredEnvVars);
      });
    });
  }

  resolveExports() {
    return resolveExportsImpl(this.provider);
  }
  ...
  resolveResource(value) {
    return BbPromise.try(() => {
      ...
      if (value.Ref) {
        ...
        // Handle condition where GetAtt has Resource.Property format
        if (typeof value[action] === 'string') {
          value[action] = value[action].split('.');
        }
        if (value[action].length !== 2) {
          throw new ServerlessError(`Attribute definition unexpected ${JSON.stringify(value)}`);
        }

        // Get the resource name
        resourceName = value[action][0];
      } else if (value['Fn::ImportValue']) {
        action = 'Fn::ImportValue';
        resourceName = value[action];
      } else {
        return value;
      }
      ...
      let resourceResult;
      if (action === 'Fn::ImportValue') {
        resourceResult = this.resolveExports().then(cfExports => {
          const result = cfExports.find(exp => exp.Name === resourceName);
          if (!result) {
            throw new ServerlessError(`Could not find import value for ${resourceName}`);
          }
          return result.Value;
        });
      } else {
        resourceResult = this.getCFResources().then(cfResources => {
          const resource = cfResources.find(x => x.LogicalResourceId === resourceName);
          if (!resource) {
            throw new ServerlessError(`Could not find the resource specified ${resourceName}`);
          }

          let cfResourceResult;

          if (action === 'Ref') {
            cfResourceResult = this.resolveResourceId(resourceName).then(result => {
              if (!_.isString(result)) {
                throw new ServerlessError(`Could not resolve Ref: ${resourceName}`);
              }
              return result;
            });
          } else {
            const resolver = envVarResolvers[resource.ResourceType];
            if (resolver && resolver[action]) {
              cfResourceResult = resolver[action](this, resource, resourceName, value[action]).then(
                result => {
                  if (!_.isString(result)) {
                    throw new ServerlessError(
                      `Could not resolve ${action}: ${resourceName} in aws`
                    );
                  }
              ...
              throw new ServerlessError(`Could not resolve ${resourceName} ${action}`);
            }
          }
          return cfResourceResult;
        });
      }
  ...
  getCFResources() {
    return this.provider
      .request('CloudFormation', 'describeStackResources', {
        StackName: `${this.serverless.service.service}-${this.serverless.service.provider.stage}`,
      })
      .then(resources => {
  ...
  resolveResourceId(resourceName) {
    return this.getCFResources().then(cf => {
      const cfResource = cf.find(x => x.LogicalResourceId === resourceName);
      if (!cfResource) {
        throw new ServerlessError(`Could not find resource ${resourceName} from deployed stack`);
      }
...
function resolveExportsImpl(provider, token, cfExports = []) {
  return provider.request('CloudFormation', 'listExports', { NextToken: token }).then(result => {
    cfExports.push(...result.Exports);
    if (result.NextToken) {
      return resolveExportsImpl(provider, result.NextToken, cfExports);
    }
